    ...
    public class OrXHoloCache : MonoBehaviour
    {
        ...
        IEnumerator CheckTargetDistance()
        {
            if (HighLogic.LoadedSceneIsFlight)
            {
                ...
                {
                    reloadWorldPos = true;
                    targetDistance = Vector3d.Distance(FlightGlobals.ActiveVessel.GetWorldPos3D(), worldPos);
        ...
        IEnumerator HoloSpawn()
        {
            if (!spawnHoloCache)
            {
                ...
                else
                {
                    ...
                    SpawnOrX_HoloCache.instance.craftFile = _craftFile;
                            ...
                            if (GUI.Button(new Rect(5, contentTop + (line * entryHeight), toolWindowWidth - 5, 20), "Create HoloCache", OrXGUISkin.button))
                            {
                                if (FlightGlobals.ActiveVessel.isEVA)
                                {
                                    if (FlightGlobals.ActiveVessel.LandedOrSplashed)
                                    {
                                        ...
                                        geoCache = true;
                                        OrXHCGUIEnabled = false;
                                        //OrXMissions.instance.StartMissionBuilder(new Vector3d(FlightGlobals.ActiveVessel.latitude, FlightGlobals.ActiveVessel.longitude, FlightGlobals.ActiveVessel.altitude), new Guid(), false, true);
                                        //HideGameUI();
                                    }
                                    else
                                    {
                ...
                foreach (ConfigNode craftNode in node.nodes)
                {
                    if (craftNode.name.Contains("HC" + hcCount + "OrXv"))
                    {
                        ...
                        else
                        {
                            ...
                            SpawnOrX_HoloCache.instance.SpawnMissionCraft(UrlDir.ApplicationRootPath + "GameData/OrX/Plugin/PluginData/"
