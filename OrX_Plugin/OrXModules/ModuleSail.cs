            ...
            if (HighLogic.LoadedSceneIsFlight && FlightGlobals.ready)
            {
                if (!this.vessel.packed)
                {
                    if (!setup)
                    {
                        ...
                        if (ls != null)
                        {
                            dlc = ls.deflectionLiftCoeff;
                            Debug.Log("[OrX Wind] SAIL ... " + this.part.name + " - Deflection Lift Coefficient: " + dlc);
                            float speed = WindGUI.instance._wi * ((1 / Vector3.Angle(WindGUI.instance.windDirection, this.part.transform.up) * dlc));
                    ...
                    else
                    {
                        ...
                        else 
                        {
                            ...
                            rigidBody.AddForce((WindGUI.instance.windDirection - this.part.transform.up).normalized * speed);
                        }
                    }
                }
            }
